...
void localservertoclient(uchar *buf, int len)   // processes any updates from the server
{
    ...
    while(p<end) switch(type = getint(p))
    {
        ...
        case SV_DIED:
        {
            ...
            if(actor==cn)
            {
                conoutf("%s knocked themselves out", d->name);
            }
            else if(actor==clientnum)
            {
                ...
                {
                    frags = -1;
                    conoutf("You betrayed a teammate (%s)", d->name);
                }
                else
                {
                    frags = 1;
                    conoutf("You betrayed %s", d->name);
                };
            ...
            else
            {
                ...
                if(a)
                {
                    ...
                    {
                        conoutf("%s betrayed their teammate (%s)", a->name, d->name);
                    }
                    else
                    {
                        conoutf("%s betrayed %s", a->name, d->name);
                    };
                };
            };
            snd_event(AK::EVENTS::DEFEATED_OGRE, d);
            d->lifesequence++;
            break;
        };
