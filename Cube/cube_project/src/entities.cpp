    ...
    loopv(ents)
    {
        ...
        else
        {
            ...
            if(e.type!=CARROT)
            {
				...
				float e_z = (float)(1 + sin(lastmillis / 100.0f + e.x + e.y) / 20);
				float e_yaw = lastmillis / 10.0f;
				renderent(e, entmdlnames[e.type-I_SHELLS], e_z, e_yaw);
				if (e.type == TELEPORT)
				{
					snd_setpos((void *)i, e.x, e.y, S(e.x, e.y)->floor + e_z, 0, e_yaw + 180.0f);
				}
            }
			else switch(e.attr2)
            {			
...
{
	 10,    50, AK::EVENTS::ITEMMAGIC,
     20,   100, AK::EVENTS::ITEMMAGIC,
      5,    25, AK::EVENTS::ITEMMAGIC,
      5,    25, AK::EVENTS::ITEMMAGIC,
     25,   100, AK::EVENTS::ITEMHEALTH100,
     50,   200, AK::EVENTS::ITEMHEALTH200,
    100,   100, AK::EVENTS::ITEMWIZARDHAT100,
    150,   150, AK::EVENTS::ITEMWIZARDHAT150,
  20000, 30000, AK::EVENTS::SUPERGEMPUP,
};
...
void checkquad(float time)
{
    if(player1->quadmillis && (player1->quadmillis -= (int)time)<0)
    {
        ...
        conoutf("Super Gem Magic is depleted");
    };
};
