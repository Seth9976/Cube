...
void splash(projectile *p, vec &v, vec &vold, int notthisplayer, int notthismonster, int qdam)
{
	...
	{
		char name[256];
		snprintf( name, 256, "End_%s_%s", guns[p->gun].name, p->owner == player1 ? "Player" : "Monster" );
		snd_event( name, p );
	}
	...
	{
		char name[256];
		snprintf( name, 256, "Hit_%s_%s", guns[p->gun].name, p->owner == player1 ? "Player" : "Monster" );
		snd_event( name, p );
	}
...
void shootv(int gun, vec &from, vec &to, dynent *d, bool local)     // create visual effect from a shot
{
	...
	{
		char name[256];
		snprintf( name, 256, "Fire_%s_%s", guns[gun].name, d == player1 ? "Player" : "Monster" );
